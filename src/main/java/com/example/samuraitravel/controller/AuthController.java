...
import com.example.samuraitravel.event.SignupEventListener;
...
public class AuthController {
	...
	public AuthController(UserService userService, SignupEventPublisher signupEventPublisher,
			SignupEventListener signupEventListener, VerificationTokenService verificationTokenService) {
	...
	public String signup(@ModelAttribute @Validated SignupForm signupForm,
		...
		//		メールアドレスが登録済みなら、BindingResultにエラー内容を追加
		...
		//	パスワードと確認用の再入力パスワードが一致しなければ、bindingResultにエラー内容を追加
		if (!userService.isSamePassword(signupForm.getPassword(),
